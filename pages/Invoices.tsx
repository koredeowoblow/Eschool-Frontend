...
import { Receipt, Search, Plus, Download, MoreVertical, CheckCircle2, Loader2, Layers, Save, School, Calendar, User, Trash2 } from 'lucide-react';
      ...
      render: (inv: Invoice) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{inv.type || 'Standard'} Billing</p>
          </div>
        </div>
      )
    },
    ...
    { header: 'Maturity', key: 'due_date', className: 'text-xs font-bold text-gray-400' },
    {
      ...
      render: (inv: Invoice) => (
        <div className="flex items-center justify-end gap-2">
          ...
          <button className="p-2 text-gray-400 hover:text-brand-primary rounded-xl"><Download size={18}/></button>
        </div>
      )
    }
  ];
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex-1 relative">
          ...
          <input 
            type="text" placeholder="Filter ledger..." value={search}
      ...
      <Modal isOpen={isManualModalOpen} onClose={() => setIsManualModalOpen(false)} title="Manual Billing Artifact">
         <form onSubmit={(e) => { e.preventDefault(); submitManual(manualInvoice); }} className="space-y-4">
            <div className="space-y-1">
              ...
              <select required value={manualInvoice.student_id} onChange={e => setManualInvoice({...manualInvoice, student_id: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl font-bold">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-1">
                {manualInvoice.items.map((item, idx) => (
                  <div key={idx} className="flex gap-2 items-center animate-in slide-in-from-right-2">
                    <input required placeholder="Item Name" value={item.name} onChange={e => updateItem(idx, 'name', e.target.value)} className="flex-1 p-2 bg-gray-50 border border-gray-100 rounded-lg text-xs font-bold" />
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1">
                 ...
                 <select required value={manualInvoice.session_id} onChange={e => setManualInvoice({...manualInvoice, session_id: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl font-bold">
                   <option value="">Select Session</option>
                   {sessionOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                 </select>
               </div>
