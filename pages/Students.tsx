...
const Students: React.FC = () => {
  ...
  const openEditModal = (student: Student) => {
    ...
    setFormData({
      ...
      admission_date: new Date().toISOString().split('T')[0],
      class_id: student.class_room?.id ? String(student.class_room.id) : '',
      section_id: '',
      school_session_id: '',
      term_id: '',
      password: '',
  ...
  const { submit, isSubmitting } = useFormSubmit(
    (data) => {
      ...
      if (isEditMode && editingStudent) {
        return api.put(`/students/${editingStudent.id}`, payload);
      }
      return api.post('/students', payload);
    },
    {
      successMessage: isEditMode ? "Student updated successfully." : "Student added successfully.",
      onSuccess: () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'status',
      render: (s: Student) => (
        <span className={`px-2 py-1 text-[9px] font-bold rounded-full uppercase ${s.status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
          }`}>
    ...
    },
    {
      ...
      render: (s: Student) => (
        <div className="flex items-center justify-end gap-1">
          ...
          <button onClick={(e) => { e.stopPropagation(); openEditModal(s); }} className="p-2 text-gray-400 hover:text-brand-primary transition-colors"><Edit2 size={18} /></button>
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onClick={() => {
          ...
          setActiveFormTab('info');
          setIsModalOpen(true);
        }} className="px-6 py-3.5 bg-brand-primary text-white rounded-2xl text-sm font-bold shadow-lg hover:bg-blue-700 transition-all active:scale-95">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={isEditMode ? "Update Student" : "Register Student"}>
