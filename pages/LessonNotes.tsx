...
const LessonNotes: React.FC = () => {
  ...
  const { submit, isSubmitting } = useFormSubmit(
    (data) => api.post('/lesson-notes', data),
    {
      onSuccess: () => {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {notes.map((note) => (
            <div key={note.id} className="card-premium p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 group">
              ...
              <div className="flex items-center gap-6">
                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 ${
                  note.status === 'Approved' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                }`}>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="New Lesson Submission">
