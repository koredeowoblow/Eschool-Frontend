...
import { Plus, Book, Calendar, Users, ChevronRight, FileText, Loader2, Inbox } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <button className="flex items-center gap-2 px-6 py-3 bg-brand-primary text-white rounded-2xl text-sm font-bold shadow-lg shadow-brand-primary/20 hover:bg-blue-700 transition-all active:scale-95">
          <Plus size={18} />
          Create Task
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {assignments.map((task) => (
            <div key={task.id} className="card-premium p-6 group hover:border-brand-primary/20 border border-transparent transition-all">
